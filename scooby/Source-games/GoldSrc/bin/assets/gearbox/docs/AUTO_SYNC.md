...
`simple-base/UI` is the editable shared source. Consumer CMake builds compile it directly; they do not copy C++ into game folders. Editing common source/headers rebuilds the affected library and relinks the consumer on its next build. Assets and shared Markdown refresh on every build of the consumer, including builds with no C++ changes. Already compiled or running modules do not update themselves: rebuild, repackage and restart/reload through the product's normal workflow.
