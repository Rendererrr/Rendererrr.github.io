...
Open **Lua > API docs** for the **LUA API DOCUMENTATION** browser beside the main GUI. Expand a section, then a function or value, to read its signature, description and examples. Rows support mouse and keyboard navigation. The dark panel has a centered display heading, a bright scrollbar and a resize grip; its size is independent of the main menu and stays inside the viewport. Use the close button or Escape while focused to dismiss it. Code is read-only, syntax-colored and selectable: Ctrl+C copies a selection and Ctrl+A selects an example. Long signatures scroll horizontally. The browser loads this complete reference with the host's game-specific API extension; optional `docs/LUA_SNIPPETS.md` adds examples. The preview supports `--page api --no-welcome`.
