...
local materials={"Lit","Flat","Chrome","Ghost","Additive","Wireframe","Glass","Glow","Fullbright","Chrome wireframe","Ghost wireframe","Neon wireframe","Galaxy","Galaxy flow"}
...
ui.subtab("visuals", "chams", "Chams", function()
    columns(function()
        ...
        ui.group("Animation",function()
            ...
            choice("Animation mode","chams.animation","style",{"Color wave","Breathing","Shimmer"})
...
ui.subtab(world,"sky","Sky",function()
    columns(function()
        ui.group("Skybox",function()
            toggle("Enable","world.skybox")
            choice("Skybox","world.skybox","style",{"Galaxy","Blue nebula","Crimson nebula"})
            slider("Brightness","world.sky_brightness","distance",0.1,2)
            ui.feature("world.sky")
        end)
        ui.next_column()
        ui.group("Sky animation",function()
            ui.feature("world.sky_animation")
            slider("Speed (Hz)","world.sky_speed","distance",0.05,1)
        end)
        ui.next_column()
        ui.group("World lighting",function()
            ui.feature("world.pulse")
            slider("World pulse speed","world.pulse_speed","distance",0.05,1)
            slider("Strength","world.pulse_amount","distance",0,0.8)
        end)
    end)
end)
