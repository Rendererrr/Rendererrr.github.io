...
`sidebar_presets` checks all three presets, absent Campaign combat registrations, Self/View routing, aliases, third-party and built-in Lua extensions, and actual mouse clicks and rejection of arrow/Space navigation. Its viewport/text matrix covers 360x280, 640x480 and 1440x900 at text scales 0.8, 1 and 2. `inline_keybind` drives real capture/Clear/mode controls with synthetic ImGui and canonical input events, including keyboard, Mouse 4, Escape, Delete, rebinding, config reload and Lua callback constraints. It also verifies that navigation keys cannot activate controls or move focus, while pointer-selected text fields retain typing/cursor editing and Ctrl+Tab cannot switch windows. `independent_activation` checks master gating, press edges, Hold release, Always, independent aim/trigger latches, capture/focus/UI lifecycle clearing and config metadata.
...
## Entity classification checkpoint

`entity_colors` verifies all 13 categories, unknown/disabled fallback, host palette priority, independent team/health settings, positions, gradients/fills/opacity, strict atomic RGBA config/Lua validation, new-to-legacy profile switching and real mouse interaction with the reusable editor. It compares warmed-up C++ allocation counts for 48 entities; this is not a game FPS measurement. See [the integration contract and pending scope](ENTITY_COLORS.md). Run the full canonical Release CTest suite for both x64 and Win32 before adopting this checkpoint.

## ESP editor and narrow dropdowns

`--esp-editor-demo --hidden --warp --no-welcome --frames 8` renders the real shared Global color editor beside narrow material/activation dropdowns using synthetic data only. Use `--width 640 --height 480` and `--width 1440 --height 900 --text-scale 2`, separate `--data` paths and `--capture` output files. Long captions stack and selected values wrap inside the control. `entity_colors` also exercises category selection, Override global inheritance and host-only category masks through the actual mouse-driven editor. No native-game input is synthesized.
